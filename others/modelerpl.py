...
def wordlist():
	...
	for infile in glob.glob(os.path.join(path, '*.txt')): #opens files from director
		try:
			...
			#Word level parsing
			#listed = regex(raw,lang).split()	
			#summary = summerize(wordgrams(listed), lang, mostfrequent)

			#Ngram level parsing
			ngrams = ngram(regex(rawtext))
			#transit = wordgrams(ngrams)
			#frequencyTable=frequencyTable+transit
			summary = summerize(model,wordgrams(ngrams),mostfrequent,wordcount,grams)
	...
	for j in grams:
		for i in range(length):
			if(j+i==length):
				break
			if ind!=0:
				temp = [rawtext[1][i:j+i],j,int(rawtext[2])]
				if len(temp[0])<2 or len(temp[0].strip())==0:
					continue
			else:
				temp = [rawtext[1][i:j+i],int(rawtext[2])]
				if len(temp)<2 or len(temp[0].strip())==0:
					continue
