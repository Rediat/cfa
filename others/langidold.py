...
def savetofile(summary, started):
	...
	# grams=grams,					#count of unique ngrams for a language
	...
	for i in summary['model']:
		mysum[i[4]] +=i[2]
		temp = [i[0],i[1],i[2],i[3],i[4],sys.getsizeof(i[1]),(i[2]/summary['wordcount'][i[4]]),(i[2]/summary['grams'][i[4]]/summary['mostfrequent'][maximum])+1, (i[2]/summary['allngrams']/summary['mostfrequent'][maximum])+1, (mysum[i[4]]/summary['wordcount'][i[4]])]		
		#['seq'[0],'term'[1],'freq'[2],'gram'[3],'bytes'[4],'lang'[5],'probability'[6],'InFreq'[7],'OvFreq'[8],'percent'[9]]
		pfile.write(str(temp[0])+","+str(temp[1])+","+str(temp[2])+","+str(temp[3])+","+str(temp[4])+","+str(temp[5])+","+str(temp[6])+","+str(temp[7])+","+str(temp[8])+","+str(temp[9])+str('\r\n'))
...
def summerize(model,transit,mostfrequent,wordcount,grams):
	...
	for j,l in enumerate(transit[1]):
		...
		temp = [j+1, l[0],l[1],len(l[0]),transit[0]]	#[seq[0],term[1],freq[2],length[3],lang[4]]
		counter+=l[1]
		j+=1
	...
	grams[lang]= uniques 				#count of unique ngrams for a language
	...
	allngrams = sum(grams.values())		#total count of all unique ngrams in all languages
	summary = dict(
					model=model,				#[seq[0],term[1],freq[2],length[3],lang[4]]
					allngrams=allngrams,		#total count of all unique ngrams in all languages
					allwords=allwords,			#all ngrams sum of frequencies in all languages
...
def myclassifier(testing,frequencyTable,grams,wrongs,mytotal,myrecall,total):
	...
	for i in testing: #[['am', [['እው', 2], ['ውነ', 2], ['ነት', 2], ['እውነ', 3], ['ውነት', 3], ['እውነት', 4]]]
		...
		# if q==20: break
		for g in grams:
			id=classify(frequencyTable,g,i[1])
			if id[0]==1:continue
			check = 1 if i[0]==id[1] else 0			
			#r.write(str(g)+","+str(i[0])+","+str(id[1])+","+str(check)+","+str(id[2])+str('\r\n'))
			if check==0:
				wrongs[i[0]][id[1]][g]+=1
			
			mytotal[i[0]][g]+=1
			total[g]+=1
			myrecall[i[0]][g]+=check
...
def overallclassify(frequencyTable,sampled,grams):
	lang = dict(am=0,ti=0,gu=0,ge=0,no=0)
	#added = dict(am=0,ti=0,gu=0,ge=0,no=0)
	#ind=1 #to indicate the overflow of grams from max length of the ngram
	lang = lang.fromkeys(lang,0) #set all values of lang to 0
	norepeat = set() ; counter=0
	for item in sampled: #['am', [['እው', 2], ['ውነ', 2], ['ነት', 2], ['እውነ', 3], ['ውነት', 3], ['እውነት', 4]]]
		#if l.to_unicode(item[0]) in norepeat: continue
		#if item[0] in norepeat: continue
		counter+=len(item)
...
def overallmyclassifier(testing,frequencyTable,grams,overallwrongs,overalltotal,overallrecall):

	for i in testing: #[['am', [['እው', 2], ['ውነ', 2], ['ነት', 2], ['እውነ', 3], ['ውነት', 3], ['እውነት', 4]]]
		
		id=overallclassify(frequencyTable,i[1],grams)
		# if id[0]==1:continue
		check = 1 if i[0]==id[0] else 0			
	
		if check==0:
			overallwrongs[i[0]][id[0]]+=1
		
		overalltotal[0]+=1
		overallrecall[i[0]]+=check